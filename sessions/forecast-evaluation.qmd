...
We now look at the forecasts from the model we visualised previously, and explore different metrics for evaluation.
...
- We generated 1000 samples from the predictive distribution for each forecast.
...
When dealing with integer forecasts, the standard PIT does not yield a uniform distribution even if the forecasts are perfectly calibrated. To address this, a randomized version of the PIT can be used [@czado2009]. For an integer-valued random variable $X$ with CDF $F_X$, the randomized PIT is defined as:
